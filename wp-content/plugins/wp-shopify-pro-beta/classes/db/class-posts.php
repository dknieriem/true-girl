...
class Posts extends \WPS\DB
{
   ...
   public function delete_posts_by_type($post_type)
   {
      ...
      $query =
         'DELETE posts, pt, pm FROM ' .
         $wpdb->prefix .
         WPS_TABLE_NAME_WP_POSTS .
         ' posts LEFT JOIN ' .
         $wpdb->prefix .
         WPS_TABLE_NAME_WP_TERM_RELATIONSHIPS .
         ' pt ON pt.object_id = posts.ID LEFT JOIN ' .
         $wpdb->prefix .
         WPS_TABLE_NAME_WP_POSTMETA .
         ' pm ON pm.post_id = posts.ID WHERE posts.post_type = %s';
   ...
   public function delete_posts_by_ids($post_ids)
   {
      ...
      $query =
         ...
         $wpdb->prefix .
         WPS_TABLE_NAME_WP_POSTMETA .
         " pm ON pm.post_id = posts.ID WHERE posts.ID IN($format)";
