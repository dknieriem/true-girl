...
use WPS\Utils;
use WPS\Transients;
use WPS\Options as Plugin_Options;
...
class Settings_Syncing extends \WPS\DB
{
    ...
    public function syncing_totals()
    {
        return [
         ...
         /* @if NODE_ENV='pro' */
         'orders' => $this->get_syncing_totals_orders(),
         'customers' => $this->get_syncing_totals_customers(),
         'webhooks' => $this->get_syncing_totals_webhooks()
         /* @endif */
      ];
    }
    ...
    public function set_syncing_totals($counts, $exclusions = [])
    {
        ...
        $products_totals = $exclusions && isset($exclusions['products']) ? 0 : $counts_products * $this->product_type_totals();
        ...
        return [
         ...
         /* @if NODE_ENV='pro' */
         'orders' => $this->update_column_single(['syncing_totals_orders' => $orders_totals], ['id' => 1]),
         'customers' => $this->update_column_single(['syncing_totals_customers' => $customers_totals], ['id' => 1]),
         'webhooks' => $this->update_column_single(['syncing_totals_webhooks' => $webhooks_totals], ['id' => 1])
         /* @endif */
      ];
    }
    ...
    public function reset_syncing_totals()
    {
        return [
         ...
         /* @if NODE_ENV='pro' */
         'orders' => $this->update_column_single(['syncing_totals_orders' => 0], ['id' => 1]),
         'customers' => $this->update_column_single(['syncing_totals_customers' => 0], ['id' => 1]),
         'webhooks' => $this->update_column_single(['syncing_totals_webhooks' => 0], ['id' => 1])
         /* @endif */
      ];
    }
    ...
    public function syncing_current_amounts()
    {
        return [
         ...
         /* @if NODE_ENV='pro' */
         'orders' => $this->get_syncing_current_amounts_orders(),
         'customers' => $this->get_syncing_current_amounts_customers(),
         'webhooks' => $this->get_syncing_current_amounts_webhooks()
         /* @endif */
      ];
    }
    ...
    public function reset_syncing_current_amounts()
    {
        return [
         ...
         /* @if NODE_ENV='pro' */
         'orders' => $this->update_column_single(['syncing_current_amounts_orders' => 0], ['id' => 1]),
         'customers' => $this->update_column_single(['syncing_current_amounts_customers' => 0], ['id' => 1]),
         'webhooks' => $this->update_column_single(['syncing_current_amounts_webhooks' => 0], ['id' => 1])
         /* @endif */
      ];
    }
