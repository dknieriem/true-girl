...
class Database extends \WPS\Processing\Vendor_Background_Process {
	...
	*/
	public function uninstall_plugin() {
		...
		if ($this->DB_Settings_General->is_free_tier() && $this->DB_Settings_General->is_pro_tier() ) {
			...
			/* @if NODE_ENV='pro' */
			$this->API_Settings_License->deactivate_license( $this->DB_Settings_License->get_license() );
			/* @endif */
	...
	*/
	public function drop_custom_tables() {
		...
		/* @if NODE_ENV='pro' */
		$results['orders'] 								= $this->DB_Orders->delete_table();
		$results['customers'] 						= $this->DB_Customers->delete_table();
		/* @endif */
	...
	*/
	public function drop_custom_migration_tables($table_suffix) {
		...
		/* @if NODE_ENV='pro' */
		$results['orders' . $table_suffix] 								= $this->DB_Orders->delete_migration_table($table_suffix);
		$results['customers' . $table_suffix] 						= $this->DB_Customers->delete_migration_table($table_suffix);
		/* @endif */
	...
	*/
	public function delete_posts_and_synced_data() {
		...
		/* @if NODE_ENV='pro' */
		$this->push_to_queue('DB_Orders');
		$this->push_to_queue('DB_Customers');
		/* @endif */
	...
	*/
	public function delete_only_synced_data() {
		...
		/* @if NODE_ENV='pro' */
		if ($selective_sync['customers'] === 1 || $selective_sync['all'] === 1) {
			$this->push_to_queue('DB_Customers');
		}

		if ($selective_sync['orders'] === 1 || $selective_sync['all'] === 1) {
			$this->push_to_queue('DB_Orders');
		}
		/* @endif */
	...
	*/
	public function get_table_delta() {
		...
		/* @if NODE_ENV='pro' */
		$tables[] = $this->DB_Orders;
		$tables[] = $this->DB_Customers;
		/* @endif */
