...
if ( $Webhooks->webhook_verified( $json_data, $Webhooks->get_header_hmac() ) ) {
  ...
  $Webhooks->on_product_update($product);
  ...
  if ( $DB_Products->product_exists($product->id) ) {
    $products_result = $DB_Products->update_items_of_type($product);
  ...
  // Hook: wps_after_product_update
  $Webhooks->after_product_update($product);
