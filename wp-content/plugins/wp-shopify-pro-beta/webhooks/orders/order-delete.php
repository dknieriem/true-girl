...
if ($Webhooks->webhook_verified($json_data, $Webhooks->get_header_hmac())) {
  ...
  // Hook: wps_on_order_delete
  $Webhooks->on_order_delete($order);

  $DB_Orders->delete_items_of_type($order);

  // Hook: wps_after_order_delete
  $Webhooks->after_order_delete($order);

} else {
  error_log('WP Shopify Error - Unable to verify webhook response from order-delete.php');
}
