...
@version       2.0.0
@since         1.0.49
@path          templates/components/pagination/breadcrumbs.php
...
if ( !is_front_page() ) {
	...
	if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
      ...
      $post_type_object = get_post_type_object($post_type);
		...
		echo '<li class="wps-breadcrumbs-item-current wps-breadcrumbs-item-archive" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><strong class="wps-breadcrumbs-current wps-breadcrumbs-archive" itemprop="name">' . ucfirst($post_type_object->rewrite['slug']) . '</strong></li>';
		...
		if ($post_type != 'post') {
			...
			echo '<li class="wps-breadcrumbs-item-cat wps-breadcrumbs-item-custom-post-type-' . $post_type . '" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a class="wps-breadcrumbs-cat wps-breadcrumbs-custom-post-type-' . $post_type . '" href="' . $post_type_archive . '" title="' . $post_type_object->labels->name . '" itemprop="item"><span class="wps-breadcrumbs-name" itemprop="name">' . ucfirst($post_type_object->rewrite['slug']) . '</span></a></li>';
