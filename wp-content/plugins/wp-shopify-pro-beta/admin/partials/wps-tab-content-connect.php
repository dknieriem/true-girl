...
<div class="tab-content <?php echo $active_tab === 'tab-connect' ? 'tab-content-active' : ''; ?> <?= $has_connection ? 'wps-connected' : ''; ?>" data-tab-content="tab-connect">
  ...
  <div class="wps-admin-section-heading-group wps-l-row wps-l-space-between">
    ...
    <h3 class="wps-status-heading wps-admin-section-heading">
      ...
      <?php if ($has_connection) { ?>
        ...
        if ($DB_Shop->get_shop_name()) { ?>
            <code class="wps-store-name"><?= $DB_Shop->get_shop_name(); ?></code>
         <?php }
  ...
  <div class="wps-admin-section">
    ...
    <p><?php printf(__('Enter your Shopify private app API keys below. <a href="%s" target="_blank">Need help finding these</a>?', WPS_PLUGIN_TEXT_DOMAIN), esc_url('https://www.youtube.com/watch?v=lYm6G35e8sI'));  ?></p>
    ...
    <form method="post" name="cleanup_options" action="options.php" id="wps-connect" class="wps-admin-form">
      ...
      <div class="wps-form-group">
        ...
        <input required <?= $has_connection ? 'disabled' : ''; ?> type="text" class="regular-text <?= $has_connection ? 'valid' : ''; ?>" id="<?= WPS_SETTINGS_GENERAL_OPTION_NAME; ?>_api_password" name="api_password" autocomplete="off" value="<?php if(!empty($connection->api_password)) echo $connection->api_password; ?>" placeholder=""> <span class="wps-help-tip wps-help-tip-inline-no-position" title="<?php esc_attr_e( 'To generate an API Password you must create a "Private App" within your Shopify account.', WPS_PLUGIN_TEXT_DOMAIN ); ?>"></span> <div class="wps-form-icon wps-animated"></div>
      ...
      <div class="wps-form-group">
        ...
        <input required <?= $has_connection ? 'disabled' : ''; ?> type="text" class="regular-text <?= $has_connection ? 'valid' : ''; ?>" id="<?= WPS_SETTINGS_GENERAL_OPTION_NAME; ?>_storefront_access_token" name="storefront_access_token" value="<?php if(!empty($connection->storefront_access_token)) echo $connection->storefront_access_token; ?>" placeholder=""> <span class="wps-help-tip wps-help-tip-inline-no-position" title="<?php esc_attr_e( 'To generate a Storefront Access Token you must create a "Private App" within your Shopify account. The Storefront Access Token is used to create the front-end cart experience.', WPS_PLUGIN_TEXT_DOMAIN ); ?>"></span><div class="wps-form-icon wps-animated"></div>
