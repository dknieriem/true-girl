...
import { checkMark } from './utils-animations'
import { get, post } from '../ws/ws'
import { getSyncingStatus } from '../ws/api/api-syncing'
...
import { isWordPressError, enable, disable } from './utils'
...
import { constructFinalNoticeList, hasConnection, returnOnlyFirstError } from './utils-data'

import { stopTimer } from './utils-timer'

import { setPostRelationships } from '../ws/wrappers'

import { clearLocalstorageCache, isConnectionInProgress, setConnectionProgress } from '../ws/localstorage'

import { isReconnectingWebhooks } from '../globals/globals-syncing'
import { isSyncingPosts } from '../globals/globals-settings'

import { syncingConfigManualCancel, syncingConfigWebhooksSuccess, syncingConfigJavascriptError, syncingConfigErrorBeforeSync } from '../ws/syncing-config'

import { activateToolButtons, toolsInit } from '../tools/tools'
...
import { populateSyncByCollections } from '../settings/settings.jsx'
...
function createProgressBar(stepName, stepMaxTotal) {
   ...
   return jQuery(
      '<div class="wps-progress-bar-wrapper" data-wps-progress-total="' +
         stepMaxTotal +
         '" id="wps-progress-bar-' +
         stepName +
         '"><span class="wps-progress-step-name">' +
         stepNamePretty +
         '</span><span class="wps-progress-step-percentage">0%</span><div class="wps-progress-bar"></div><span class="dashicons dashicons-yes"></span></div>'
   )
}
...
function updateProgressBarTotal(stepTotal, stepName) {
   jQuery('#wps-progress-bar-' + stepName)
      .data('wps-progress-total', stepTotal)
      .attr('data-wps-progress-total', stepTotal)
}
...
async function progressStatus() {
   ...
   */
   if (statusError) {
      ...
      return cleanUpAfterSync(syncingConfigJavascriptError(statusError))
   }
   ...
   if (!syncingTotalsMatch(status.data.syncing_totals, status.data.syncing_current_amounts)) {
      ...
      */
      ...
      if (isReconnectingWebhooks()) {
         ...
         return cleanUpAfterSync(syncingConfigWebhooksSuccess())
      }
...
async function afterPostRelationships(callback) {
   if (!isSyncing() && !isConnecting()) {
      return cleanUpAfterSync(syncingConfigManualCancel())
   }

   var [postsStatusError, postsStatus] = await to(get(endpointSyncingStatusPosts()))

   if (postsStatusError) {
      return cleanUpAfterSync(syncingConfigJavascriptError(postsStatusError))
   }
...
async function afterWebhooksRemoval(callback) {
   ...
   if (webhooksStatusError) {
      return cleanUpAfterSync(syncingConfigJavascriptError(webhooksStatusError))
   }
