...
import { unbindDisconnectForm, unbindConnectForm } from '../forms/forms'

import { disable, enable, removeTrueAndTransformToArray, isWordPressError, isConnected, hideLoader } from '../utils/utils'
...
import { returnOnlyFailedRequests, addToWarningList, setConnectionStatus, returnOnlyFirstError } from '../utils/utils-data'

import { post } from '../ws/ws'

import { deleteWebhooks } from '../ws/api/api-webhooks'

import { endpointConnectionDelete, endpointConnectionCheck, endpointToolsClearAll, endpointToolsClearSynced } from '../ws/api/api-endpoints'

import { afterWebhooksRemoval, cleanUpAfterSync, manuallyCanceled } from '../utils/utils-progress'

import { clearAllCache, noConnectionReset } from '../ws/wrappers'

import { clearLocalstorageCache, removeConnectionProgress, setWebhooksReconnect } from '../ws/localstorage'

import { connectInit, getConnectionFormData } from '../connect/connect'

import { removeExistingData, syncOn, syncOff } from '../ws/syncing'

import { syncingConfigJavascriptError, syncingConfigErrorBeforeSync, syncingConfigDisconnection, syncingConfigManualCancel } from '../ws/syncing-config'
...
function disconnectionFormSubmitHandler(e) {
    ...
    */
      ...
      /* @if NODE_ENV='pro' */
      insertCheckmark()
      setConnectionStepMessage('Removing webhooks ...', '(Please wait, this might take 30 seconds or so)')
      ...
      */
         ...
         jQuery('.wps-status-heading .wps-store-name').remove()
