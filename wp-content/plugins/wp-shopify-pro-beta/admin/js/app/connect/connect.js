...
import { noConnectionReset, clearAllCache } from '../ws/wrappers'
import { saveCounts } from '../ws/api/api-syncing'
import { endpointConnection, endpointConnectionCheck, endpointToolsClearSynced } from '../ws/api/api-endpoints'
import { getShop } from '../ws/api/api-shop'
import { syncingConfigErrorBeforeSync, syncingConfigJavascriptError, syncingConfigSavedConnectionOnly, syncingConfigManualCancel, syncingConfigLiteSync } from '../ws/syncing-config'
import { unbindDisconnectForm, formatConnectorFormData, formConnectionRules } from '../forms/forms'
import { enable, disable, showSpinner, containsProtocol, cleanDomainURL, containsPathAfterShopifyDomain, isWordPressError, getDataFromArray } from '../utils/utils'
import { initSyncingTimer } from '../utils/utils-timer'
import { startProgressBar, appendProgressBars, progressStatus, afterWebhooksRemoval, cleanUpAfterSync, afterDataRemoval, manuallyCanceled } from '../utils/utils-progress'
import { setConnectionStepMessage, showAdminNotice, updateModalButtonText, updateModalHeadingText, insertCheckmark, ejectConnectorModal, createModal, hideAdminNoticeByType } from '../utils/utils-dom'
import { filterOutAnyNotice, filterOutSelectiveSync, filterOutEmptySets, setConnectionStatus, returnOnlyFirstError } from '../utils/utils-data'
import { post } from '../ws/ws'
import { deleteWebhooks } from '../ws/api/api-webhooks'
import { getPublishedProductIds } from '../ws/api/api-products'
import { clearLocalstorageCache, setWebhooksReconnect, setCancelSync, setConnectionProgress } from '../ws/localstorage'
...
function connectionFormSubmitHandler(form) {
   ...
   return new Promise(async (resolve, reject) => {
      ...
      */
      ...
      var [saveConnectionError, saveConnectionData] = await to(post(endpointConnection(), { connection: getConnectionFormData(form) }))
      ...
      */
      ...
      var [serverCheckError, serverCheckData] = await to(
         post(endpointConnectionCheck(), {
            type: 'shopify',
      ...
      */
      afterDataRemoval(async () => {
         ...
         /* @if NODE_ENV='pro' */
         var [deleteWebhooksError, deleteWebhooksData] = await to(deleteWebhooks())
         ...
         if (isWordPressError(deleteWebhooksData)) {
            cleanUpAfterSync(syncingConfigErrorBeforeSync(returnOnlyFirstError(deleteWebhooksData)))
            return resolve()
         }
         ...
         afterWebhooksRemoval(async () => {
            ...
            */
            ...
            var [saveCountsError, saveCountsResponse] = await to(
               saveCounts({
                  counts: allCounts
               })
            )
...
export { connectInit, prepareBeforeSync, getConnectionFormData }
