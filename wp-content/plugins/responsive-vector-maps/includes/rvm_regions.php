...
$output .= '<tbody>';
$output .= '<th>' . __( 'Subdivision', RVM_TEXT_DOMAIN ) . '</th>';
$output .= '<th>Link</th>';
$output .= '<th id="rvm_region_bg_col">' . __( 'Background', RVM_TEXT_DOMAIN ) . '</th>';
$output .= '<th>Label popup</th>';
$output .= '<th id="rvm_region_mouse_hover_over_col">' . __( 'Activate Mouse Over Background - <span class="rvm_small_text">hold  [SHIFT] key for multiple select</span>', RVM_TEXT_DOMAIN ) . '</th>';
...
foreach ( $array_regions as $region ) {
            ...
            $output .= '<td class="rvm_text_center"><p><input  type="checkbox" class="rvm_region_checkbox" name="' . strval( $region[ 1 ] ) . '[]"    ' . checked( 'checked', $regionsparams_array[ 'field_region_mouse_hover_over_colour' ] , false ) . ' ></p></td>';
            $output .= '</tr>';
