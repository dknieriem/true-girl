            ...
            if ( isset( $attr[ 'mapid' ] ) && is_numeric( $attr[ 'mapid' ] ) ) { // Check if mapid attr exists and if is numeric
                        ...
                        if ( !empty( $postid ) ) {
                                    ...
                                    $rvm_selected_map = get_post_meta( $map_id, '_rvm_mbe_select_map', true );
                                          ...
                                          $array_regions = rvm_include_custom_map_settings( $map_id  ,  $rvm_selected_map );
                                              if( isset( $array_regions ) && !empty( $array_regions ) && is_array($array_regions) ) {
                                                ...
                                                $output .= '<div  data-plugin="responsive-vector-maps" data-ver="' . RVM_VERSION . '" class="map-container rvm-map-container" id="' . $rvm_selected_map . '-map-' . $map_id . '"  ' . $rvm_mbe_width . '>';
                                                      ...
                                                      if ( is_array( $regionsparams_array ) && !empty( $regionsparams_array ) ) {                                                
                                                            if( isset( $regionsparams_array[ 'field_region_onclick_action' ] ) && $regionsparams_array[ 'field_region_onclick_action' ] == 'open_label_onto_default_card' ) {
                                                                  ...
                                                                  $region[ 1 ] = rvm_region_match_when_numeric( $region[ 1 ] );
                                                                                                      $output .= '<div id="rvm-default-card-' . $region[ 1 ] . '" class="rvm-card-container rvm-default-card-container rvm-hide-card-container">' . stripslashes_deep ( $regionsparams_array[ 'field_region_popup' ] ) .'<div class="rvm-close-default-card-container">x</div></div>'; 
                                                            }
                                                      }
                                                }
                                                ...
                                                foreach ( $array_regions as $region ) {
                                                            ...
                                                            if ( is_array( $regionsparams_array ) && !empty( $regionsparams_array ) ) {
                                                                        ...
                                                                        if( isset( $regionsparams_array[ 'field_region_onclick_action' ] ) && $regionsparams_array[ 'field_region_onclick_action' ] == 'open_label_onto_default_card' ) {
                                                                                                                  ...
                                                                                                                  $output .= 'if(code == "' .  $region[ 1 ] . '") { document.getElementById("rvm-default-card-' . $region[ 1 ] . '").classList.toggle("rvm-hide-card-container"); }';                                                                  
                                                                        }
                                                                        // If user wants to display is own div or any other selector
                                                                              ...
                                                                              $output .= 'var rvm_custom_selector = document.getElementById("' . $regionsparams_array[ 'field_region_link' ] . '");';
                                                                              ...
                                                                              $output .= 'if(code == "' .  $region[ 1 ] . '") {
                                                                                    ...
                                                                                    if ( rvm_custom_selector.className == "rvm-active-custom-card" ) {
                                              ...
                                              if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                                               ...
                                               $output .= '<script type="text/javascript">( function($) {';
                                                ...
                                                $output .= '$( ".rvm-close-default-card-container").click(function (){';
                                                $output .= '$( ".rvm-close-default-card-container").parents(".rvm-default-card-container").addClass("rvm-hide-card-container");';//parents() ensure to search through all the ancestors
                                                $output .= '});';
                                                $output .= '})(jQuery);</script>';
                                                /**
                                    ...
                                    if ( isset( $rvm_selected_map ) && !empty( $rvm_selected_map ) ) {
                                        $output.= '<img  src="' . RVM_IMG_PLUGIN_DIR . '/' . $rvm_selected_map . '-map-replacement.png" alt="' . esc_attr( $rvm_selected_map ) . '" />';  
                                    }

                                    else {
                                          $output.= __( 'Unfortunately map is no longer available', RVM_TEXT_DOMAIN ) ;
                                    }
