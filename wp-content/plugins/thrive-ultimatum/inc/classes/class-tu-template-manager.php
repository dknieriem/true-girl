...
class TU_Template_Manager extends TU_Request_Handler {
	...
	protected function config() {
		$this->config = include TVE_Ult_Const::plugin_path() . 'editor-templates/_config.php';
	}
	...
	public static function get_templates( $design_type = '' ) {
		...
		foreach ( $templates as $tpl => $tpl_data ) {
			...
			$templates[ $tpl ]['thumbnail'] = TVE_Ult_Const::plugin_url( 'editor-templates/' . $design_type . '/thumbnails/' . $tpl . '.png' );
		}
	...
	public function api_get_saved( $return = false ) {
		...
		$img     = '<img src="' . TVE_Ult_Const::plugin_url() . 'editor-templates/%s/thumbnails/%s" width="166" height="140"/>';
