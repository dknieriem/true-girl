 ...
 * @param $file string
 */
function tve_ult_tcb_ajax_load( $file ) {
	switch ( $file ) {
		case 'tve_ult_templates':
			include TVE_Ult_Const::plugin_path() . 'editor/lightbox/design_templates.php';
			exit;
			break;
	}
}
...
function tve_ult_enqueue_design_scripts( $for_design = null ) {
	...
	if ( ! empty( $config['css'] ) ) {
		tve_ult_enqueue_style( $css_key, TVE_Ult_Const::plugin_url( 'editor-templates/css/' . TU_Template_Manager::type( $for_design['post_type'] ) . '/' . $config['css'] ) );
	}
	...
	$css = array(
		$css_key => TVE_Ult_Const::plugin_url( 'editor-templates/css/' . TU_Template_Manager::type( $for_design['post_type'] ) . '/' . $config['css'] . '?ver=' . TVE_Ult_Const::PLUGIN_VERSION ),
	);
...
function tve_ult_enqueue_default_scripts() {
	...
	$GLOBALS['tve_ult_res']['js']['tve_frontend']               = tve_editor_js() . '/thrive_content_builder_frontend' . $js_suffix;
