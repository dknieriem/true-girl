  ...
  // arrays
    ...
    if( $('.post-type-tf_stats').length ) {
        ...
        $('#wpbody').off().on('click', '.icon-add, .remove' ,function(){
          var idD = $(this).closest('.cmb-row'); 
          if( $(this).hasClass('fa-times') ) {
            $(idD).find('.item-title .rain-icon-prev i').attr('class', '');
            $(idD).find('.menu-item-settings .menu-item-icon-markup').val('');
          } else {
            $('#icons,#icons-wrap').addClass('active');
            $('#icons').off().on('click', 'i, div img', function(){
