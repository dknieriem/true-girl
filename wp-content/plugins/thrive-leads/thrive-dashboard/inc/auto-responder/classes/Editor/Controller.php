			...
			case 'api':
				...
				$data     = array(
					'fields_table'        => $renderer->apiFieldsTable( array(
						...
						'show_order'           => true
					), $this->_param( 'api_fields_order', array() ), $connection_config ),
					'connection_config'   => $connection_config,
					'show_submit_options' => apply_filters( 'tve_autoresponder_show_submit', true ),
					'custom_messages'     => $custom_messages,
					'create_account'     => isset($_POST['create_account']) ? $_POST['create_account'] : 0,
					'extra_fields'     => isset($_POST['extra_fields']) ? $_POST['extra_fields'] : array(),
				);
				$content  = $this->_view( 'dashboard-api', $data );

				$fields = $this->_param( 'api_fields' );

				if(!array_key_exists('wordpress', $connection_config) && ! empty($fields['password'])) {
					$response['elements'] = $renderer->getApiFields();
				} else {
					$response['elements']           = $fields && ! empty($fields['password']) ? $this->_param( 'api_fields' ) : $renderer->getApiFields();
				}
