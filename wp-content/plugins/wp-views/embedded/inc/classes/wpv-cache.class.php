...
class WPV_Cache {
	...
	static function generate_post_relationship_cache( $cache_post_relationship = array(), $id_posts = array(), $relationship_to_cache = array() ) {
		...
		foreach ( $id_posts as $id_post_to_cache ) {
			$association_query_condition_role = Toolset_Relationship_Role::role_from_name( $direct_relationship['role_target'] ); 
			$association_query_conditions[] = $association_query->element_id_and_domain( $id_post_to_cache, Toolset_Element_Domain::POSTS, $association_query_condition_role );
		}
